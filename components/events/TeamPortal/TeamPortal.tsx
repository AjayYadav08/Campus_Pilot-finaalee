...
const TeamPortal: React.FC<TeamPortalProps> = (props) => {
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-[9px] font-black px-1.5 py-0.5 rounded-md uppercase tracking-widest ${
                    activeTeam.status === 'Open' ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-slate-100 text-slate-500 border border-slate-200'
                  }`}>
                    {activeTeam.status}
                  </span>
                </div>
            ...
            {teamHosts[activeTeam.id] === 'me' && (
              <button
                ...
                className={`px-3 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all border ${
                  activeTeam.status === 'Open' ? 'bg-amber-50 text-amber-600 border-amber-100' : 'bg-emerald-50 text-emerald-600 border-emerald-100'
                }`}
              >
  ...
  if (showCreateTeamForm) return <CreateTeamForm setShowCreateTeamForm={setShowCreateTeamForm} newTeamData={newTeamData} setNewTeamData={setNewTeamData} handleCreateTeam={handleCreateTeam} />;
